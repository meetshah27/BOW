...
import { AlertCircle, CheckCircle, Clock, Eye, Loader, Search, Store, Trash2, XCircle } from 'lucide-react';
...
const VendorApplicationsManagement = () => {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const res = await api.get('/vendor-applications/applications');
      if (!res.ok) throw new Error('Failed to fetch vendor applications');
  ...
  const updateStatus = async (application, status) => {
    try {
      ...
      const res = await api.put(`/vendor-applications/applications/${application.id}/status`, {
  ...
  const deleteApplication = async (application) => {
    ...
    try {
      const res = await api.delete(
        `/vendor-applications/applications/${application.id}?applicantEmail=${encodeURIComponent(application.applicantEmail)}`
      );
      const data = await res.json().catch(() => ({}));
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-gray-600">Loading vendor applications...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Review and approve/reject vendor applications</p>
        </div>
                ...
                filteredApplications.map((app) => (
                  <tr key={`${app.applicantEmail}:${app.id}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-sm text-gray-500">{app.businessType || '—'}</div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="bg-gray-50 p-4 rounded-xl">
                  ...
                  <div className="text-sm text-gray-700 space-y-1">
                    ...
                    <div>
                      <strong>Type:</strong> {selectedApplication.businessType || '—'}
                    </div>
                    <div>
              ...
              <div className="bg-gray-50 p-4 rounded-xl">
                <h4 className="font-semibold text-gray-900 mb-2">Products / Notes</h4>
                <div className="text-sm text-gray-700 space-y-2">
                  <div>
                    <strong>Products:</strong>
                    <div className="mt-1 whitespace-pre-wrap">{selectedApplication.productsDescription || '—'}</div>
                  </div>
                  <div>
                    <strong>Notes:</strong>
