...
/**
 * BOW Stripe Secure Setup Script
 * 
 * This script helps you securely store Stripe keys in DynamoDB
 * using the existing AWS infrastructure.
 */

require('dotenv').config();
const readline = require('readline');
const { keyManager } = require('./config/key-management-simple');
...
async function setupStripeSecure() {
  console.log('🔐 BOW Stripe Secure Setup');
  console.log('==========================');
  console.log('');
  console.log('This script will help you securely store Stripe API keys');
  console.log('in DynamoDB using your existing AWS infrastructure.');
  console.log('');
  ...
  try {
    ...
    console.log('');
    console.log('💾 Storing keys securely in DynamoDB...');

    await keyManager.storeEncryptedKey('stripe_secret_key', stripeSecretKey, true);
    await keyManager.storeEncryptedKey('stripe_publishable_key', stripePublishableKey, false);
    await keyManager.storeEncryptedKey('stripe_webhook_secret', stripeWebhookSecret, true);
    ...
    if (testResult) {
      ...
      console.log('   1. Your Stripe keys are now securely stored in DynamoDB');
      ...
      console.log('   - Keys are stored in DynamoDB with encryption flags');
    ...
    console.error('   - Ensure your AWS user has DynamoDB permissions');
...
if (require.main === module) {
  setupStripeSecure().catch(error => {
    console.error('❌ Unexpected error:', error);
    process.exit(1);
  });
}

module.exports = { setupStripeSecure };
