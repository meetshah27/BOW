...
router.get('/campaigns', async (req, res) => {
  try {
    console.log('[GET /api/newsletter/campaigns] Fetching all campaigns');
    console.log('[GET /api/newsletter/campaigns] NewsletterCampaign model loaded:', !!NewsletterCampaign);
    
    if (!NewsletterCampaign) {
      console.log('[GET /api/newsletter/campaigns] Model not available');
      return res.status(500).json({ error: 'NewsletterCampaign model not available' });
    }
    
    const campaigns = await NewsletterCampaign.getAllCampaigns();
    console.log(`[GET /api/newsletter/campaigns] Found ${campaigns.length} campaigns`);
    
    if (campaigns.length === 0) {
      console.log('[GET /api/newsletter/campaigns] No campaigns found - returning empty array');
      return res.json({ campaigns: [] });
    }
    
    res.status(200).json({
      campaigns: campaigns.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
    });
  } catch (error) {
    console.error('[GET /api/newsletter/campaigns] Error:', error);
    console.error('[GET /api/newsletter/campaigns] Error details:', {
      name: error.name,
