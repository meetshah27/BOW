...
// GET - Test endpoint (public, no auth required)
...
router.post('/register', async (req, res) => {
  try {
    console.log('[POST /users/register] Registration request received');
    console.log('[POST /users/register] Request body:', req.body);
    console.log('[POST /users/register] Request headers:', req.headers);
...
router.post('/login', async (req, res) => {
  try {
    console.log('[POST /users/login] Login request received');
    console.log('[POST /users/login] Request body:', req.body);
    console.log('[POST /users/login] Request headers:', req.headers);
    const { email, password } = req.body;
    if (!email || !password) {
      console.log('[POST /users/login] Missing email or password');
      return res.status(400).json({ message: 'Email and password are required' });
    }
    if (!User) {
      console.log('[POST /users/login] User model not available');
