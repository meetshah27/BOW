...
router.get('/', async (req, res) => {
  try {
    console.log('[GET /api/stories] Story model loaded:', !!Story);
    
    if (Story) {
      console.log('[GET /api/stories] Fetching stories from DynamoDB...');
      const stories = await Story.findAll();
      console.log('[GET /api/stories] Found', stories.length, 'stories in DynamoDB');
      
      if (stories.length === 0) {
        console.log('[GET /api/stories] No stories found in DynamoDB - returning empty array');
        return res.json([]);
      }
      
      res.json(stories);
    } else {
      console.log('[GET /api/stories] Story model not available - using fallback sample data');
      // Fallback to sample data
      res.json(sampleStories);
    }
  } catch (error) {
    console.error('[GET /api/stories] Error fetching stories:', error);
    console.error('[GET /api/stories] Error details:', {
      name: error.name,
