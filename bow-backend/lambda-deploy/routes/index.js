...
router.get('/api/hero', async (req, res) => {
  try {
    console.log('[GET /api/hero] Fetching hero settings');
    // Try to use DynamoDB Hero model, fallback to default if not available
    let Hero;
    try {
      Hero = require('../models-dynamodb/Hero');
      console.log('[GET /api/hero] Hero model loaded:', !!Hero);
    } catch (error) {
      console.log('[GET /api/hero] DynamoDB Hero model not available, using fallback mode');
      Hero = null;
    }

    if (Hero) {
      try {
        console.log('[GET /api/hero] Fetching hero settings from DynamoDB...');
        const heroSettings = await Hero.getSettings();
        console.log('[GET /api/hero] Successfully fetched hero settings');
        res.json(heroSettings);
      } catch (dynamoError) {
        console.error('[GET /api/hero] Error fetching hero settings:', dynamoError);
        console.error('[GET /api/hero] Error details:', {
        ...
        console.log('[GET /api/hero] Using fallback data');
      ...
      console.log('[GET /api/hero] Using fallback data (no model)');
      res.json(defaultHeroSettings);
    }
  } catch (err) {
    ...
    if (MissionMedia) {
      try {
        ...
        const client = new DynamoDBClient({
          ...
          // Only set credentials if NOT in Lambda
...
router.get('/api/mission-media', async (req, res) => {
  try {
    console.log('[GET /api/mission-media] Fetching mission media');
    console.log('[GET /api/mission-media] MissionMedia model loaded:', !!MissionMedia);
    
    if (MissionMedia) {
      try {
        console.log('[GET /api/mission-media] Fetching mission media from DynamoDB...');
        const missionMedia = await MissionMedia.getMissionMedia();
        console.log('[GET /api/mission-media] Successfully fetched mission media');
        res.json(missionMedia);
      } catch (dynamoError) {
        console.error('[GET /api/mission-media] Error fetching mission media:', dynamoError);
        console.error('[GET /api/mission-media] Error details:', {
          name: dynamoError.name,
          message: dynamoError.message,
          code: dynamoError.code,
          statusCode: dynamoError.$metadata?.httpStatusCode
        ...
        if (dynamoError.name === 'ResourceNotFoundException') {
          console.error('[GET /api/mission-media] DynamoDB table does not exist!');
        }
        ...
        console.log('[GET /api/mission-media] Using fallback data');
        res.json(defaultMissionMedia);
      }
    } else {
      console.log('[GET /api/mission-media] Using fallback mode (no DynamoDB models)');
    ...
    console.error('[GET /api/mission-media] Error:', err);
    res.status(500).json({ 
      error: 'Failed to fetch mission media',
      message: err.message,
      details: process.env.NODE_ENV === 'development' ? err.stack : undefined
    });
  }
