...
router.get('/subscribers', async (req, res) => {
  try {
    console.log('[GET /api/newsletter/subscribers] Fetching all subscribers');
    console.log('[GET /api/newsletter/subscribers] Newsletter model loaded:', !!Newsletter);
    
    if (!Newsletter) {
      console.log('[GET /api/newsletter/subscribers] Model not available');
      return res.status(500).json({ error: 'Newsletter model not available' });
    }
    
    const subscribers = await Newsletter.getAllSubscribers();
    console.log(`[GET /api/newsletter/subscribers] Found ${subscribers.length} subscribers`);
    
    if (subscribers.length === 0) {
      console.log('[GET /api/newsletter/subscribers] No subscribers found - returning empty array');
    ...
    console.error('[GET /api/newsletter/subscribers] Error details:', {
      name: error.name,
