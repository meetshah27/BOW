...
router.get('/applications', async (req, res) => {
  try {
    console.log('[GET /api/memberships/applications] Fetching membership applications');
    console.log('[GET /api/memberships/applications] Membership model loaded:', !!Membership);
    
    if (Membership) {
      try {
        // Try to use database first
        const applications = await Membership.findAll();
        console.log(`[GET /api/memberships/applications] Found ${applications.length} applications in DynamoDB`);
        ...
        if (sortedApplications.length === 0) {
          console.log('[GET /api/memberships/applications] No applications found - returning empty array');
          return res.json([]);
        }
        
        res.json(sortedApplications);
      } catch (dbError) {
        console.error('[GET /api/memberships/applications] Database error:', dbError);
        console.error('[GET /api/memberships/applications] Error details:', {
          name: dbError.name,
        ...
        res.status(500).json({ 
          error: 'Failed to fetch membership applications from database',
          message: dbError.message
        });
      }
    } else {
      console.log('[GET /api/memberships/applications] Membership model not available - using sample data');
    ...
    console.error('[GET /api/memberships/applications] Error:', error);
    res.status(500).json({ 
