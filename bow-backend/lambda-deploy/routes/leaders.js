...
router.get('/', async (req, res) => {
  try {
    ...
    console.log('[GET /api/leaders] Leader model loaded:', !!Leader);
    
    if (!Leader) {
      console.log('[GET /api/leaders] Leader model not available');
      return res.status(500).json({ error: 'Leader model not available' });
    }
    
    const leaders = await Leader.getAll();
    console.log(`[GET /api/leaders] Found ${leaders.length} active leaders`);
    
    if (leaders.length === 0) {
      console.log('[GET /api/leaders] No leaders found - returning empty array');
      return res.json([]);
    }
    res.json(leaders);
  } catch (error) {
    console.error('[GET /api/leaders] Error fetching leaders:', error);
    console.error('[GET /api/leaders] Error details:', {
      name: error.name,
