...
router.get('/', async (req, res) => {
  try {
    console.log('[GET /api/founder-content] Fetching founder content');
    console.log('[GET /api/founder-content] FounderContent model loaded:', !!FounderContent);
    
    if (!FounderContent) {
      console.log('[GET /api/founder-content] FounderContent model not available');
      return res.status(500).json({ error: 'FounderContent model not available' });
    }
    
    const founderContent = await FounderContent.getSettings();
    console.log('[GET /api/founder-content] Successfully fetched founder content');
    res.json(founderContent);
  } catch (error) {
    console.error('[GET /api/founder-content] Error fetching founder content:', error);
    console.error('[GET /api/founder-content] Error details:', {
      name: error.name,
    ...
    res.status(500).json({ 
      error: 'Failed to fetch founder content',
      message: error.message,
...
router.post('/', async (req, res) => {
  try {
    ...
    console.error('[POST /api/founder-content] Error updating founder content:', {
      message: error.message,
      stack: error.stack,
      requestBody: req.body,
      details: error.details || error.cause || {}
    });
    res.status(500).json({ error: 'Failed to update founder content', message: error.message });
  }
});
