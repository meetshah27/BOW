...
router.get('/opportunities', async (req, res) => {
  try {
    console.log('[GET /api/volunteer-opportunities/opportunities] Fetching all opportunities');
    console.log('[GET /api/volunteer-opportunities/opportunities] VolunteerOpportunity model loaded:', !!VolunteerOpportunity);
    
    if (!VolunteerOpportunity) {
      console.log('[GET /api/volunteer-opportunities/opportunities] Model not available');
      return res.status(500).json({ error: 'VolunteerOpportunity model not available' });
    }
    
    const opportunities = await VolunteerOpportunity.getAllOpportunities();
    console.log(`[GET /api/volunteer-opportunities/opportunities] Found ${opportunities.length} opportunities`);
    
    if (opportunities.length === 0) {
      console.log('[GET /api/volunteer-opportunities/opportunities] No opportunities found - returning empty array');
      return res.json({ opportunities: [] });
    }
    
    res.status(200).json({
      opportunities: opportunities.sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
    });
  } catch (error) {
    console.error('[GET /api/volunteer-opportunities/opportunities] Error:', error);
    console.error('[GET /api/volunteer-opportunities/opportunities] Error details:', {
      name: error.name,
...
router.get('/opportunities/active', async (req, res) => {
  try {
    console.log('[GET /api/volunteer-opportunities/opportunities/active] Fetching active opportunities');
    console.log('[GET /api/volunteer-opportunities/opportunities/active] VolunteerOpportunity model loaded:', !!VolunteerOpportunity);
    
    if (!VolunteerOpportunity) {
      console.log('[GET /api/volunteer-opportunities/opportunities/active] Model not available');
      return res.status(500).json({ error: 'VolunteerOpportunity model not available' });
    }
    
    const opportunities = await VolunteerOpportunity.getActiveOpportunities();
    console.log(`[GET /api/volunteer-opportunities/opportunities/active] Found ${opportunities.length} active opportunities`);
    
    if (opportunities.length === 0) {
      console.log('[GET /api/volunteer-opportunities/opportunities/active] No active opportunities found - returning empty array');
      return res.json({ opportunities: [] });
    }
    ...
    console.error('[GET /api/volunteer-opportunities/opportunities/active] Error:', error);
    console.error('[GET /api/volunteer-opportunities/opportunities/active] Error details:', {
      name: error.name,
      message: error.message,
      code: error.code,
      statusCode: error.$metadata?.httpStatusCode
