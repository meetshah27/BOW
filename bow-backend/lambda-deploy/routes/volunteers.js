...
router.get('/opportunities', async (req, res) => {
  try {
    console.log('[GET /api/volunteers/opportunities] Fetching volunteer opportunities');
    
    // Try to use DynamoDB if available, otherwise use sample data
    if (Volunteer) {
      // This route might be using a different model or approach
      // For now, return sample data but log it
      console.log('[GET /api/volunteers/opportunities] Using sample data (Volunteer model available but opportunities endpoint uses sample)');
    }
    
    res.json(sampleOpportunities);
  } catch (error) {
    console.error('[GET /api/volunteers/opportunities] Error fetching volunteer opportunities:', error);
    res.status(500).json({ 
