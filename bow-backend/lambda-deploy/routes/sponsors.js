...
router.get('/', async (req, res) => {
  try {
    console.log('[GET /api/sponsors] Fetching active sponsors');
    console.log('[GET /api/sponsors] Table name:', TABLES.SPONSORS);
    ...
    console.log(`[GET /api/sponsors] Found ${(result.Items || []).length} active sponsors`);
    
    if ((result.Items || []).length === 0) {
      console.log('[GET /api/sponsors] No sponsors found - returning empty array');
      return res.json([]);
    }
    
    res.json(result.Items || []);
  } catch (error) {
    console.error('[GET /api/sponsors] Error fetching sponsors:', error);
    console.error('[GET /api/sponsors] Error details:', {
      name: error.name,
