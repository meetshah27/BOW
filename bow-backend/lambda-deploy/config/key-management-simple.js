...
function resolveAppConfigTable() {
  if (process.env.APP_CONFIG_TABLE_NAME) {
    return process.env.APP_CONFIG_TABLE_NAME;
  }

  const prefix = process.env.DYNAMODB_TABLE_PREFIX;
  if (prefix) {
    const needsSeparator = !prefix.endsWith('-') && !prefix.endsWith('_');
    return `${prefix}${needsSeparator ? '-' : ''}app_config`;
  }

  return 'bow-app_config';
}

const APP_CONFIG_TABLE = process.env.APP_CONFIG_TABLE_NAME || resolveAppConfigTable();
