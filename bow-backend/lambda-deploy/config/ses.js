...
const sesConfig = {
  ...
  // Only set credentials if NOT in Lambda and they're provided
...
console.log('   Running in Lambda:', isLambda);
if (!isLambda) {
  console.log('   Access Key ID set:', !!process.env.AWS_ACCESS_KEY_ID);
  console.log('   Secret Access Key set:', !!process.env.AWS_SECRET_ACCESS_KEY);
}

// Check if required environment variables are set (only for non-Lambda)
...
<body>
  ...
  static async sendContactFormNotification({ name, email, subject, message }) {
    const adminSubject = `New Contact Form Submission: ${subject}`;
    const htmlContent = this.getContactFormTemplate({ name, email, subject, message });
    const textContent = this.getContactFormTextTemplate({ name, email, subject, message });
    
    // Send to admin email
    return await this.sendEmail({
      to: SES_CONFIG.FROM_EMAIL,
...
<head>
    ...
    <style>
        ...
        .message-box { background: #fff; padding: 20px; border: 1px solid #e9ecef; border-radius: 8px; margin: 20px 0; white-space: pre-wrap; }
        .footer { background: #f8f9fa; padding: 20px; text-align: center; color: #666; font-size: 14px; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>This is an automated notification from the Beats of Washington contact form.</p>
            <p>Please reply directly to <a href="mailto:${email}">${email}</a> to respond to this inquiry.</p>
        </div>
    </div>
</body>
</html>`;
  ...
  // Get contact form text template
  static getContactFormTextTemplate({ name, email, subject, message }) {
    return `
...
---
This is an automated notification from the Beats of Washington contact form.
