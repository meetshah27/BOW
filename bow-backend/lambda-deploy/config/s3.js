...
const rawBucketName = process.env.S3_BUCKET_NAME || 'bow-media-bucket';
const sanitizedBucketName = rawBucketName.trim();

if (/\s/.test(sanitizedBucketName)) {
  console.warn(`⚠️ S3 bucket name contains whitespace: "${rawBucketName}"`);
}

console.log('   Bucket Name:', sanitizedBucketName);
const cdnDomain = process.env.CDN_DOMAIN || '';
if (cdnDomain) {
  console.log('   CDN domain detected:', cdnDomain);
}
...
const S3_CONFIG = {
  BUCKET_NAME: sanitizedBucketName,
  REGION: process.env.AWS_REGION || 'us-west-2', // Updated to us-west-2
  FOLDERS: {
    EVENTS: 'events',
    GALLERY: 'gallery',
