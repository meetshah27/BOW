...
class Story {
  ...
  static async findAll() {
    try {
      console.log('[Story.findAll] Scanning table:', TABLES.STORIES);
      const allItems = [];
      ...
      do {
        ...
        console.log('[Story.findAll] Scan result:', {
        ...
        if (result.Items && result.Items.length > 0) {
          // Filter out items without title (these are likely not stories)
          const storyItems = result.Items.filter(item => item.title && item.title.trim() !== '');
          allItems.push(...storyItems);
          console.log('[Story.findAll] Filtered items:', {
            total: result.Items.length,
            withTitle: storyItems.length,
            withoutTitle: result.Items.length - storyItems.length
          });
        }
      ...
      console.log('[Story.findAll] Total story items found:', allItems.length);
      
      if (allItems.length === 0) {
        console.log('[Story.findAll] No story items with titles found in table');
        return [];
      }
      
      const stories = allItems.map(item => new Story(item));
      console.log('[Story.findAll] Returning', stories.length, 'stories');
      return stories;
    } catch (error) {
      console.error('[Story.findAll] Error finding all stories:', error);
      console.error('[Story.findAll] Error details:', {
        name: error.name,
